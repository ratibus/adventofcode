...
foreach ($numbers as $number) {
    foreach ($boards as $boardIndex => $board) {
        if (!isset($boardsUnmarkedSum[$boardIndex])) {
            continue;
        }
        ...
        for ($i = 0; $i < 5; $i++) {
            ...
            if ($nbHorizontalNulls === 5 && isset($boardsUnmarkedSum[$boardIndex])) {
                printf("Horizontal win. Score: %d\n", $number * $boardsUnmarkedSum[$boardIndex]);
                unset($boardsUnmarkedSum[$boardIndex]);
            }
            if ($nbVerticalNulls === 5 && isset($boardsUnmarkedSum[$boardIndex])) {
...
// The answer is the last score displayed
